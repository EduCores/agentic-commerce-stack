...
- Model calls run through a fallback chain defined by `buildModelChain()` in `prisma/starshop-prompts.ts` (configured model → `nvidia/nemotron-3.5-lightning:free` → `nvidia/nemotron-3-super-120b-a12b:free` → `qwen/qwen3.8-27b:free`); `runAgent`/`streamAgent`/`directChat`/intent router retry the next model on 402/429/5xx/network errors. When adding a model id, also add it to `ALLOWED_MODELS` (`agent/index.ts`) and `FLOW_MODELS` (`src/components/flow/types.ts`). `qwen/qwen3.8-27b:free` is the test model (`scripts/test-agent.ts` swaps it in and restores the original); its upstream can 429 and the chain falls through to Nemotron automatically.
- Emails transaccionales inmediatos (2FA/reset login, confirmación de pedido, etc.) dispatchean por el workflow `email-agent` (`src/workflows/email-agent.ts`, slug `email-agent`) vía `dispatchEmailEvent({ trigger, to, subject, text })` — awaited (`startWorkflow(..., { wait: true })`), logs en WorkflowRun/OrderStepLog/EmailLog. Sin `RESEND_API_KEY` hace mock. El cron de carritos (`/api/cron/emails`) y `EmailSequence` no pasan por aquí (tienen su propio motor con waits). Tras cambiar `emailAgentGraph`, republica con `npx tsx scripts/sync-router-graph.ts`.
