...
- Policies/shipping source of truth: the real business site is PrestaShop at `www.starshop.cl` — Envíos y Devoluciones `https://www.starshop.cl/portal/index.php?id_cms=1&controller=cms`, Condiciones de uso `https://www.starshop.cl/portal/index.php?id_cms=3&controller=cms`. Neither `starshop-rho` (demo) nor `starshop.cl/politicas` have policy pages (404). `scrapeWebsite` uses `r.jina.ai` (not `cc.jina.ai`, which is dead) and returns `ok:false` on HTTP ≥ 400 instead of the error page HTML.
