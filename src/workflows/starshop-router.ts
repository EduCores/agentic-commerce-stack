...
export const starShopRouterGraph = {
  nodes: [
    ...
    { id: "crew-support", type: "base", position: { x: 300, y: 280 }, data: { label: "3 Soporte general", description: "Responde políticas, envíos y garantías según las tarifas y tiempos oficiales de StarShop.", detail: "Tools: calculatePricing, navigateTo. RM $3.990 (gratis >$49.990 en 24-48h), regiones $4.990-$9.990, garantía hasta 3 años y 30 días devolución. Si no resuelve, escala a ventas@starshop.cl.", type: "agent_decision", status: "idle", agent: "StarShop Support Agent", tools: STARSHOP_CREW_TOOLS.general_inquiry, prompt: STARSHOP_CREWS.general_support.prompt, intent: "general_inquiry", model: STARSHOP_CREWS.general_support.model } },
    ...
    { id: "crew-return", type: "base", position: { x: 600, y: 280 }, data: { label: "3 Gestión de devoluciones", description: "Evalúa la devolución según política oficial StarShop (30 días) y envía email con los pasos.", detail: "2 agents/2 tasks. Tools: sendEmail (return_update), searchProducts (SKU), orderTracking. Aprueba dentro de 30 días sin costo.", type: "cancel", status: "idle", agent: "Returns Policy Evaluator", tools: STARSHOP_CREW_TOOLS.return_request, prompt: STARSHOP_CREWS.handle_return.prompt, intent: "return_request", model: STARSHOP_CREWS.handle_return.model } },
    ...
    { id: "crew-tracking", type: "base", position: { x: 150, y: 400 }, data: { label: "3 Seguimiento de pedido (WISMO)", description: "Informa el estado del pedido: Order.status + WorkflowRun.currentStep + StepLogs.", detail: "Tools: orderTracking (lee Order/WorkflowRun/StepLog), sendEmail (notificación). Pide orderId/email si falta.", type: "webhook", status: "idle", agent: "Order Tracker", tools: STARSHOP_CREW_TOOLS.order_tracking, prompt: STARSHOP_CREWS.order_tracking.prompt, intent: "order_tracking", model: STARSHOP_CREWS.order_tracking.model } },
    { id: "crew-validate", type: "base", position: { x: 450, y: 400 }, data: { label: "3 Validar stock y precio", description: "Nodo transversal: valida el stock y calcula el total con despacho por región.", detail: "Invocado por Search y Checkout. Tools: checkStock (SKU exacto), calculatePricing (sku, qty, región). Evita alucinar stock/precio. Usa tier pricing (5/10 uds).", type: "reserve_stock", status: "idle", tools: ["checkStock", "calculatePricing"], prompt: "Nodo transversal de validación: ante cualquier SKU, verifica stock real con checkStock y calcula el total con despacho por región con calculatePricing. Si el stock no alcanza, informa las unidades disponibles y ofrece alternativas equivalentes. Nunca confirmes precio ni stock sin llamar a las tools." } },
