...
export const starShopRouterGraph = {
  nodes: [
    ...
    { id: "crew-checkout", type: "base", position: { x: 0, y: 280 }, data: { label: "3 Checkout Guide", description: "Acompaña pago paso a paso: valida stock + precio con flete y cierra con checkout/processPurchase.", detail: "Tools: checkStock, calculatePricing, checkout, processPurchase, navigateTo, sendEmail. Valida antes de confirmar; si falla stock ofrece alternativas. Converge a Confirm Order.", type: "fulfill", status: "idle", agent: "StarShop Checkout Guide", tools: STARSHOP_CREW_TOOLS.checkout_support, prompt: STARSHOP_CREWS.checkout_guide.prompt, intent: "checkout_support", model: STARSHOP_CREWS.checkout_guide.model } },
    { id: "crew-compare", type: "base", position: { x: 150, y: 280 }, data: { label: "3 Compare Prices Crew", description: "Compara precio StarShop vs externo scrapeando URLs (Jina) y presenta tabla.", detail: "2 agents/2 tasks. Tools: searchProducts (precio base), scrapeWebsite (Jina Reader), calculatePricing. Si no puede scrapear, muestra precio StarShop y explica.", type: "agent_decision", status: "idle", agent: "StarShop Price Analyst", tools: STARSHOP_CREW_TOOLS.price_comparison, prompt: STARSHOP_CREWS.compare_prices.prompt, intent: "price_comparison", model: STARSHOP_CREWS.compare_prices.model } },
    { id: "crew-support", type: "base", position: { x: 300, y: 280 }, data: { label: "3 General Support", description: "Responde políticas, envíos, garantías; scrapea starshop.cl/politicas si falta info.", detail: "Tools: scrapeWebsite, navigateTo. Nunca inventa políticas; si no encuentra, escala a ventas@starshop.cl.", type: "agent_decision", status: "idle", agent: "StarShop Support Agent", tools: STARSHOP_CREW_TOOLS.general_inquiry, prompt: STARSHOP_CREWS.general_support.prompt, intent: "general_inquiry", model: STARSHOP_CREWS.general_support.model } },
    ...
    { id: "crew-validate", type: "base", position: { x: 450, y: 400 }, data: { label: "3 Validate Stock & Pricing", description: "Nodo transversal: valida stock y calcula total con despacho por región.", detail: "Invocado por Search y Checkout. Tools: checkStock (SKU exacto), calculatePricing (sku, qty, región). Evita alucinar stock/precio. Usa tier pricing (5/10 uds).", type: "reserve_stock", status: "idle", tools: ["checkStock", "calculatePricing"] } },
