...
export function HomeFunnelCard() {
  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-xl border border-card-border bg-card-background p-5">
        ...
        <p className="mt-2 text-xs text-text-tertiary">Sin datos aún.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-card-border bg-card-background p-5">
      <div className="h-1.5 rounded-full bg-gradient-to-r from-violet-400 to-primary-500" />
      <div className="mt-3 flex items-center justify-between gap-2">
        <h3 className="text-sm font-semibold tracking-[-0.2px] text-text-primary">Embudo de conversión</h3>
        <Badge color="primary">{totalConv}% conversión total</Badge>
      </div>
